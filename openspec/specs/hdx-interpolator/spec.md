...
The plugin SHALL define `HdxInterpolator` in `pkg/plugin/interpolator.go` that satisfies `sqlds.Interpolator` (`Interpolate(ctx, *SQLDatasource, *sqlutil.Query, json.RawMessage) (string, error)`). The implementation SHALL be safe for concurrent use across queries.

#### Scenario: Interface conformance

- **GIVEN** a `*HdxInterpolator` value
- **WHEN** assigned to a `sqlds.Interpolator` interface variable
- **THEN** the assignment SHALL compile without error
- **AND** the `Interpolate` method SHALL accept the upstream signature
...
When invoked through the `sqlds.Interpolator` interface, the implementation SHALL overlay `query.RawSQL`, `query.TimeRange`, and `query.Interval` from the upstream `*sqlutil.Query` onto the unmarshalled `models.HdxQuery`, so the sqlds-derived runtime context takes precedence over anything carried in `rawJSON`.
