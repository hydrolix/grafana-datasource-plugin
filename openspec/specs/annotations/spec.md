...
Exposes the Hydrolix datasource as a Grafana annotation source. Owns the annotation-query lifecycle hooks (`prepareQuery`, `getDefaultQuery`) and the rules for how annotation requests interact with shared datasource state (ad-hoc filter cache, cached request options). Result-column-to-event-field binding is delegated to Grafana's built-in annotation field-mapping UI; the plugin does not impose a column convention.
