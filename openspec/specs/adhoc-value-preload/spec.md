# adhoc-value-preload Specification

## Purpose

Defines how the plugin populates ad-hoc filter key/value dropdowns: the
value-preload query shape (`topK` aggregation, array and map column
handling, synthetic empty/null entries) and the guardrails applied to every
query the metadata provider issues — an execution-time circuit breaker on
the driver settings channel, `break` overflow semantics and a timerange
backstop in the SQL text, and a trailing-24h, 5-minute-rounded scan window —
so high-cardinality preloads return bounded partial results instead of
hanging. Grafana never passes the user's typed text to `getTagValues`, so
this single preload query is the only chance to return useful values.
