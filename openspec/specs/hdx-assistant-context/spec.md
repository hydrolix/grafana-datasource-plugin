# hdx-assistant-context Specification

## Purpose
What the plugin publishes to Grafana Assistant: page context items (SQL, schema, time range, datasource host), the query-editor entry point, and the explain-error action — with every surface inert when Assistant is unavailable. Delivered by change add-assistant-mcp-support.
## Requirements
