...
- **BREAKING (internal Go API)**: pin `github.com/hydrolix/sqlds/v5` to the pseudo-version for commit `ef925e1` (`v5.0.0-20260613133402-ef925e15e15e`). The fork at this revision no longer carries `HydrolixDatasource`, `Connector` as a public type with Hydrolix behaviour, the AST interpolator, or any Hydrolix macros — only the extension-point interfaces.
...
- Coordinated-shipment note: this change is buildable in isolation only if accompanying changes C3-C7 land in the same merge window — references to `sqlds.GetMacroCTEs`, `sqlds.CTE`, and the runtime macros disappear from the fork at `ef925e1` and reappear in `pkg/plugin/` only via C5-C7. The OpenSpec changes describe the conceptual decomposition; PR strategy may stack or coalesce them.
