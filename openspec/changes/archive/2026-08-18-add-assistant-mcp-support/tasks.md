...
- [x] 11.7 Gate `QueryWithAssistantButton` on `assistantAvailable` alongside `ExplainErrorButton`. It renders ungated at `QueryEditor.tsx:289` and self-gates inside the SDK, so the DOM outcome was already correct — but `hdx-assistant-context`'s *Assistant surfaces are inert when Assistant is unavailable* puts the determination on the plugin ("SHALL determine Assistant availability before rendering any Assistant-related UI"), and delegating it made the guarantee the SDK's. Both toolbar surfaces now share one gate; the §11.2 unavailable-case test asserts all three surfaces and was verified to fail against the ungated arrangement.
