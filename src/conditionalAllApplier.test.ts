...
describe('ConditionalAllApplier', () => {
    ...
    const cases: Array<{
    ...
    }> = [
        {
            ...
            query: 'select foo from table where $__conditionalAll(bar in ($bar), $bar);',
            variables: [
        ...
        },
        {
            ...
            query: 'select foo from table where $__conditionalAll(bar in ($bar), $bar) and $__conditionalAll(bar in ($bar2), $bar2);',
            variables: [
                { name: 'bar', current: { value: `'val1', 'val2'` } } as any,
