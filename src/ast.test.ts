...
describe("ast getColumnValuesStatement", () => {
  ...
  test("should return statement for statusCode for table with variables with condition", () => {
    ...
    expect(result).toBe(
      "SELECT statusCode, COUNT(statusCode) as count  FROM sample.log WHERE $__timeFilter(ts) AND $__adHocFilter() AND toString(statusCode) like '2%' GROUP BY statusCode ORDER BY count DESC LIMIT 100"
    );
  });
});
