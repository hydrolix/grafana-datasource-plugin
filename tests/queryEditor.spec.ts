...
test("smoke: should render query editor", async ({
    ...
    const dsConfigPage = await configPageSteps.createDatasourceConfigPage("queryEditor render", createDataSourceConfigPage)
...
test("should provide editor hints", async ({
    ...
    const configPageSteps = new ConfigPageSteps(page);
    const dsConfigPage = await configPageSteps.createDatasourceConfigPage("queryEditor hints", createDataSourceConfigPage)
    await configPageSteps.fillTestNativeDatasource()
    await configPageSteps.saveSuccess(dsConfigPage)
    ...
    await context.route('**/*/api/ds/query*', async (route, request) => {
        ...
        if (body?.includes('SELECT name FROM system.tables WHERE engine = \'TurbineStorage\' AND database = ')) {
            const json = {
                "results": {
                    "MD": {
                        "status": 200,
        ...
        } else if (body?.includes('SELECT DISTINCT database as project FROM system.tables WHERE engine = \'TurbineStorage\'')) {
            const json = {
                "results": {
                    "MD": {
                        "status": 200,
    ...
    expect(suggestions).toEqual(expect.arrayContaining([
        '*',
        '$__interval_s()',
        '$__timeInterval(timeColumn)', '$__timeInterval_ms(timeColumn)'
    ]));
