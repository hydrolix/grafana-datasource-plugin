...
test("smoke: should render config editor", async ({
    ...
    await ConfigPageSteps.createDatasourceConfigPage("render", createDataSourceConfigPage);
    ...
    await expect(configPage.path(), "path shouldn't be visible for native (default)").not.toBeVisible()
    ...
    await expect(configPage.path(), "path should be visible for http connection").toBeVisible()
    ...
    await expect(configPage.adHocKeysQuery()).not.toBeVisible()
    await expect(configPage.adHocValuesQuery()).not.toBeVisible()
    ...
    await expect(configPage.adHocKeysQuery()).toBeVisible()
    await expect(configPage.adHocValuesQuery()).toBeVisible()
...
test('"Mandatory fields validation', async ({
    ...
    const configPage = ConfigPageSteps.getLocator(page)
    await ConfigPageSteps.createDatasourceConfigPage("mandatory fields", createDataSourceConfigPage);
...
test('"Save & test" should be successful when configuration is valid', async ({
    ...
    const dsConfigPage = await configPageSteps.createDatasourceConfigPage("valid config", createDataSourceConfigPage);
...
test('"Save & test" should fail when configuration is invalid', async ({
    ...
    const configPageSteps = new ConfigPageSteps(page)
    const dsConfigPage = await configPageSteps.createDatasourceConfigPage("invalid config", createDataSourceConfigPage);
