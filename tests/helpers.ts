...
import {DataSourceConfigPage, expect, GrafanaPage, PanelEditPage, TimeRangeArgs} from "@grafana/plugin-e2e";
...
class ElementContext {
    ...
    reset(loc: Locator): Locator {
        return loc.getByRole("button", {name: "reset"}).or(loc.locator('button')).first();
    }
        ...
        get(target: Page, propKey, receiver) {
            return (...args) => {
                ...
                for (i = 0; i < chains.length; i++) {
                    ...
                    if (["Input", "Item", "Switch", "Expandable", "Alert", "Timeselect", "Reset", "Button"].indexOf(chain) >= 0) {
                        break;
                    }
 ...
 */
export const timerangeSet = async ({from, to, zone}: TimeRangeArgs, page: GrafanaPage) => {
    ...
    try {
        ...
        await page.ctx.page.locator('[aria-controls="TimePickerContent"]').last().click();
    }

    if (zone) {
        const changeTimeSettingsButton = semver.gte(page.ctx.grafanaVersion, '11.0.0')
        ...
        await page.getByGrafanaSelector(TimeZonePicker.containerV2).locator('input').fill(zone);
        await page.getByGrafanaSelector(TimeZonePicker.containerV2).getByRole("option").filter({hasText: zone}).click();
 ...
 */
export const queryTextSet = async (refId: string, query: string, page: PanelEditPage): Promise<void> => {
...
export class ConfigPageSteps {
    ...
    static getLocator(page: Page): ConfigPageLocator {
        return new Proxy(page as any, pageHandler(allLabels.components.config.editor)) as ConfigPageLocator;
    }
     ...
     */
    @step
    static async createDatasourceConfigPage(name: string, createDataSourceConfigPage: (args: CreateDataSourcePageArgs) => Promise<DataSourceConfigPage>) {
     ...
     */
    @step
    async createDatasourceConfigPage(name: string, createDataSourceConfigPage: (args: CreateDataSourcePageArgs) => Promise<DataSourceConfigPage>) {
        return createDataSourceConfigPage({
            type: "hydrolix-hydrolix-datasource",
            name: name,
            deleteDataSourceAfterTest: false
    ...
    async fillTestNativeDatasource() {
        ...
        await this.configPageLocator.useDefaultPortSwitch().uncheck({force: true});
