...
test.beforeEach(async ({dashboardPage, createDataSourceConfigPage}) => {
    if (panelEditPage === undefined) {
        const dsConfigPage = await ConfigPageSteps.createDatasourceConfigPage("macroFunctions tests", createDataSourceConfigPage)
...
[
    {
        ...
        from: '2025-04-10 00:20:00', to: '2025-04-10 23:59:59',
        expected: [
    ...
    {
        name: 'toTime',
        query: 'select * from e2e.macros where datetime >= $__fromTime and datetime <= $__toTime',
        from: '2025-04-09 00:00:00', to: '2025-04-09 00:29:00',
        expected: [
    ...
    {
        name: 'toTime_ms',
        query: 'select * from e2e.macros where datetime >= $__fromTime and datetime <= $__toTime_ms',
        from: '2025-04-09 00:00:00', to: '2025-04-09 00:29:00',
        expected: [
            ['2025-04-09 00:00:00', '2025-04-09', '0'],
            ['2025-04-09 00:10:00', '2025-04-09', '1000'],
    ...
    },
    {
        ...
        from: '2025-04-10 00:00:00', to: '2025-04-10 23:59:59',
        expected: [
    ...
    },
    {
        ...
        from: '2025-04-09 01:10:59', to: '2025-04-09 01:11:00',
        expected: [
    ...
    {
        name: 'timeInterval',
        query: 'select $__timeInterval(datetime) as datetime, max(date) as date, max(v1) as v1 from e2e.macros where $__timeFilter_ms(datetime) group by datetime',
        from: '2025-04-09 00:00:00', to: '2025-04-19 23:59:59',
        maxDataPoints: 400,
        expected: [
    ...
    },
    {
        ...
        query: 'select $__timeInterval_ms(datetime) as datetime, max(date) as date, max(v1) as v1 from e2e.macros where $__timeFilter_ms(datetime) group by datetime',
        maxDataPoints: 400,
        from: '2025-04-09 00:00:00', to: '2025-04-19 23:59:59',
        expected: [
            ['2025-04-09 00:00:00', '2025-04-09 00:00:00', '2000'],
    ...
    {
        name: 'interval_s',
        query: 'select toStartOfInterval(datetime, INTERVAL $__interval_s second) as datetime, max(date) as date, max(v1) as v1 from e2e.macros where $__timeFilter_ms(datetime) group by datetime',
        maxDataPoints: 400,
        from: '2025-04-09 00:00:00', to: '2025-04-19 23:59:59',
        expected: [
            ['2025-04-09 00:00:00', '2025-04-09 00:00:00', '2000'],
    ...
    },
    {
        name: 'round',
        query: 'select * from e2e.macros where $__timeFilter_ms(datetime)',
        from: '2025-04-10 00:20:30', to: '2025-04-11 00:29:30',
        round: '2m',
        expected: [
            ['2025-04-10 00:20:00', '2025-04-10', '2000'],
            ['2025-04-10 00:30:00', '2025-04-10 00:00:00', '3000'],
    ...
    test(`testing ${name}`, async () => {
        ...
        await timerangeSet({from, to, zone: "Coordinated Universal Time"}, panelEditPage)

        if (maxDataPoints) {
            // set interval to 30m for TimeInterval
            await panelEditPage.getByGrafanaSelector('Query operation row title').click()
            await panelEditPage.ctx.page.locator('xpath=//label[contains(text(), "Max data points")]/../div[1]//input').fill(maxDataPoints.toString())
            await panelEditPage.getByGrafanaSelector('Query operation row title').click()
        }
        if (round){
        ...
        await expect(panelEditPage.panel.fieldNames).toContainText(['datetime', 'date', 'v1']);
        expected.forEach(async (v) => {
            await expect(panelEditPage.panel.data).toContainText(v);
        })
        await expect(panelEditPage.panel.data).not.toContainText(['2025-04-08 23:59:59', '2025-04-08', '-1']);
        await expect(panelEditPage.panel.data).not.toContainText(['2025-04-11 00:00:01', '2025-04-11', '-1']);
    });
});
